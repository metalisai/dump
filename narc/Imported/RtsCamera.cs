...
public class RtsCamera : MonoBehaviour
{
    ...
    protected void LateUpdate()
    {
        ...
        if (Smoothing)
        {
            _currRotation = Mathf.LerpAngle(_currRotation, Rotation, GameTime.RealDeltaTime * RotationDampening);
            _currDistance = Mathf.Lerp(_currDistance, Distance, GameTime.RealDeltaTime * ZoomDampening);
            _currTilt = Mathf.LerpAngle(_currTilt, Tilt, GameTime.RealDeltaTime * TiltDampening);
            _target.transform.position = Vector3.Lerp(_target.transform.position, LookAt, GameTime.RealDeltaTime * MoveDampening);
        }
        else
        {
            _currRotation = Rotation;
