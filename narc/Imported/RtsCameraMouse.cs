...
public class RtsCameraMouse : MonoBehaviour
{
    ...
    protected void Update()
    {
        ...
        if (AllowZoom && !EventSystem.current.IsPointerOverGameObject())
        {
            var scroll = Input.GetAxisRaw(ZoomInputAxis);
            _rtsCamera.Distance -= scroll * ZoomSpeed * GameTime.RealDeltaTime;
        }
        ...
        if (Input.GetKey(MouseOrbitButton))
        {
            ...
            {
                // pan
                var panX = -1 * Input.GetAxisRaw("Mouse X") * PanSpeed * GameTime.RealDeltaTime;
                var panZ = -1 * Input.GetAxisRaw("Mouse Y") * PanSpeed * GameTime.RealDeltaTime;
            ...
            else
            {
                ...
                if (AllowTilt)
                {
                    ...
                    _rtsCamera.Tilt -= tilt * TiltSpeed * GameTime.RealDeltaTime;
                }

                if (AllowRotate)
                {
                    var rot = Input.GetAxisRaw(RotateInputAxis);
                    _rtsCamera.Rotation += rot * RotateSpeed * GameTime.RealDeltaTime;
                }
            }
        }
        ...
        if (AllowScreenEdgeMove && (!_rtsCamera.IsFollowing || ScreenEdgeMoveBreaksFollow))
        {
            ...
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(0, 0, MoveSpeed * GameTime.RealDeltaTime);
            }
            else if (Input.mousePosition.y < ScreenEdgeBorderWidth)
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(0, 0, -1 * MoveSpeed * GameTime.RealDeltaTime);
            }

            if (Input.mousePosition.x > (Screen.width - ScreenEdgeBorderWidth))
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(MoveSpeed * GameTime.RealDeltaTime, 0, 0);
            }
            else if (Input.mousePosition.x < ScreenEdgeBorderWidth)
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(-1 * MoveSpeed * GameTime.RealDeltaTime, 0, 0);
            }
